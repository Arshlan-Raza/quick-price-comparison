...
const normalizeSourceToPlatform = (source: string): Platform | null => {
  const lower = source.toLowerCase();
  if (lower.includes("blinkit")) return "blinkit";
  if (lower.includes("zepto")) return "zepto";
  if (lower.includes("swiggy") || lower.includes("instamart")) return "swiggy";
  if (lower.includes("bigbasket")) return "bigbasket";
  if (lower.includes("dunzo")) return "dunzo";
  return null;
};
...
export const ShopProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const fetchProducts = async () => {
    try {
      ...
      const possibleUrls = ["http://localhost:5000/api/grouped-products"];
      ...
      if (fetchAttemptsRef.current < maxFetchAttempts) {
        ...
        console.log(
          `Will retry in ${delay}ms (attempt ${fetchAttemptsRef.current} of ${maxFetchAttempts})`
        );
        setTimeout(() => {
          fetchProducts();
        }, delay);
  ...
  useEffect(() => {
    ...
    if (selectedCategory !== "All") {
      ...
      console.log(
        `After category filter (${selectedCategory}): ${filtered.length} products`
      );
    }
    ...
    if (selectedPlatform) {
      filtered = filtered.filter((product) =>
        product.prices.some(
          (price) => price.platform === selectedPlatform && price.available
        )
      );
      console.log(
        `After platform filter (${selectedPlatform}): ${filtered.length} products`
      );
    }
    ...
    if (searchQuery.trim() !== "") {
      ...
      console.log(
        `After search filter (${searchQuery}): ${filtered.length} products`
      );
    }
  ...
  const getCartTotal = (platform?: Platform) => {
    return cart.reduce((total, item) => {
      ...
      if (platform) {
        ...
        priceInfo = item.product.prices.find(
  ...
  const getConsolidatedTotal = (platform: Platform) => {
    return cart.reduce((total, item) => {
      const priceInfo = item.product.prices.find(
  ...
  const saveUserCart = async () => {
    ...
    try {
      ...
      if (!saved) {
        console.error(
          `Failed to save cart to any endpoint. Details:\n${errorMessage}`
        );
      }
    } catch (error) {
  ...
  const loadUserCart = async () => {
    ...
    try {
      ...
      if (!response) {
        console.error(
          `Failed to load cart from any endpoint. Details:\n${errorMessage}`
        );
        return;
      }
  ...
  const getBestPlatformForCart = () => {
    ...
    const availablePlatforms = platformStats.filter(
