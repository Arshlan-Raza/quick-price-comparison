          ...
          <div key={analysis.platform.id} className="flex flex-col">
              ...
              className={`
                ...
                ${
                  analysis.platform.id === bestPlatform?.platform.id
                    ? "bg-platform-" + analysis.platform.id + "/5"
                    : ""
                }
              `}
            ...
            >
              <div className="flex items-center gap-2">
                <span
                  className={`font-medium platform-${analysis.platform.id}`}
                >
                ...
                {!analysis.allAvailable && (
                  <span className="text-xs text-muted-foreground">
                    ({analysis.availableCount}/{analysis.totalCount} items
                    available)
                  </span>
                )}
              </div>
                      ...
                      >
                        ...
                        <div className="text-right">
                          {item.available ? (
                            ...
                            (() => {
                              const alternatives = item.product.prices.filter(
                                (p) =>
                                  p.available &&
                                  p.platform !== analysis.platform.id
                              );
                              ...
                              return bestAlternative ? (
                                <div className="text-sm text-muted-foreground text-right flex flex-col items-end gap-1">
                                  ...
                                  <div className="text-xs">
                                    Try on{" "}
                                    <span className="font-semibold">
                                      {bestAlternative.platform}
                                    </span>
                                    <br />₹
                                    {(
                                      bestAlternative.price * item.quantity
                                    ).toFixed(2)}
                                  </div>
                                  <Button
                                    size="sm"
